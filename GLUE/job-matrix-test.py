...
#if list_files[0].lower().endswith('zip'):
if type_file == 'zip':
    #When files is ZIP, we download y directory temporal in glue for proccess
    # Group multiple files ZIP
    # for n in config_table["filename"]:
    tmp_path = 'TMP_ZIP/' + list_files[0].replace('.zip', '').replace('.ZIP', '').split('_')[0]

    s3 = boto3.client("s3")
    response = s3.list_objects_v2(Bucket=config_table["bucket_name"], Prefix=tmp_path)

    ### Delete TMP folder ###
    if 'Contents' in response.keys():
        for object in response['Contents']:
            s3.delete_object(Bucket=config_table["bucket_name"], Key=object['Key'])

    for file in list_files:
        path_s3_source = f'{config_table["path"]}/{file}'
        #Download Zip
        obj = s3.get_object(
            Bucket=config_table["bucket_name"],
            Key=f'{config_table["path"]}/{file}'
        )
        ...
        with ZipFile(objbuffer) as zip:
            ...
            for filename in filenames:
                with zip.open(filename) as file:
                    tmp_file = f'{tmp_path}/{filename.split("/")[-1]}'
                    s3.upload_fileobj(file, config_table["bucket_name"], tmp_file)
                    
                    path = f's3://{config_table["bucket_name"]}/{tmp_file}'
    ...
    for file in list_files:
        path = f's3://{config_table["bucket_name"]}/{config_table["path"]}/{file}'
        ...
        if True:
            ...
            df1.write \
                                ...
                                .option("tempdir", table_temporal) \
                                .option("aws_iam_role", iam_redshift) \
                                .option("postactions", iam_redshift) \
                                .mode("overwrite") \
                                .save()
        else:
            df1.write \
        ...
        #option("preactions",query). #postactions
